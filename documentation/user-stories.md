...
- `npm run build` doit produire un dossier `docs/` de **fichiers HTML/CSS/JS/assets
...
- [x] `output: 'static'` explicite dans `astro.config.mjs`, aucun adaptateur SSR/serverless installé — `npm run build` produit un `docs/` en HTML pur (voir `outDir` dans `astro.config.mjs`, hébergement GitHub Pages), servable sans runtime Node (voir [§0bis](#0bis-contrainte-technique--export-100--statique-urls-seo-friendly)).
...
- [x] Convention de nommage `placeholder-*` documentée dans `documentation/README.md` ou un `CONTRIBUTING.md` court.
...
> **Retirée à la demande du produit.** Le sélecteur manuel a été supprimé du header
> (`SeasonSwitcher.astro` retiré) : le thème saisonnier est désormais **entièrement
> automatique**, déterminé par la date du jour, sans réglage possible par le
> visiteur. Cette US est donc caduque — conservée ici pour l'historique.
...
**Priorité** : Retirée · **Réf. PRD** : §6.1, §17 (R6)

### US-031 — Détermination automatique de la saison

**En tant que** visiteur, **je veux** que la saison affichée corresponde toujours à la
date réelle, **afin de** voir un site cohérent avec le moment de ma visite.

**Critères d'acceptation**
- [x] Règle de date par défaut correcte pour les 4 saisons (§6.2), calculée au build.
- [x] Script client < 2 ko, actif uniquement si le build affiché a plus de 24 h (§6.4) —
  aucun réglage manuel (ni sélecteur, ni paramètre d'URL, ni préférence mémorisée) :
  retiré à la demande du produit, la saison n'est jamais choisie par le visiteur.
