...
| **Convention** | Chaque fichier remplace `public/medias/placeholder/placeholder-<slug>.svg` par un vrai média au même usage (voir `documentation/user-stories.md` §0 et US-070). |
...
Une fois un fichier prêt, il suffit de le déposer dans `public/medias/` (hors du dossier `placeholder/`) et de mettre à jour le `slug` correspondant dans le code — voir `documentation/user-stories.md` US-070 pour le suivi du remplacement.
